...
def goplus_security(chain, address, cached):
    ...
    try:
        ...
        try:
            ...
            reason = f"GoPlus request failed: {type(exc).__name__}: {str(exc)[:180]}"
            print(f"GOPLUS_SECURITY: chain={chain_name} request_error={reason}")
            return security_result_unknown(reason)
        api_code = raw.get("code") if isinstance(raw, dict) else None
        api_message = raw.get("message") if isinstance(raw, dict) else None
        print(f"GOPLUS_SECURITY: chain={chain_name} code={api_code} message={str(api_message or '')[:120]}")
