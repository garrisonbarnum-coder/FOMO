...
def acceleration_signals(item, old):
    """Detect meaningful acceleration/deceleration between 5-minute scans.
    These are early-warning heuristics, not predictions."""
    old = old if isinstance(old, dict) else {}
    score = num(item.get("opportunityScore"))
    old_score = num(old.get("opportunityScore"))
    volume = num(item.get("volume24h"))
    old_volume = num(old.get("volume24h"))
    liquidity = num(item.get("liquidityUsd"))
    old_liquidity = num(old.get("liquidityUsd"))
    buys = num(item.get("buys1h"))
    sells = num(item.get("sells1h"))
    total = buys + sells
    buy_ratio = buys / total if total else 0
    price1h = num(item.get("priceChange1h"))

    volume_delta = (volume - old_volume) / old_volume if old_volume > 0 else 0
    liquidity_delta = (liquidity - old_liquidity) / old_liquidity if old_liquidity > 0 else 0
    score_delta = score - old_score if old else 0

    positive = []
    negative = []
    if score_delta >= 8: positive.append(f"FOMO score +{score_delta:.0f}")
    if volume_delta >= 0.25: positive.append(f"24h volume +{volume_delta*100:.0f}%")
    if liquidity_delta >= 0.10: positive.append(f"liquidity +{liquidity_delta*100:.0f}%")
    if buy_ratio >= 0.62 and total >= 20: positive.append(f"buy pressure {buy_ratio*100:.0f}%")
    if price1h >= 5: positive.append(f"1h price +{price1h:.1f}%")

    if score_delta <= -10: negative.append(f"FOMO score {score_delta:.0f}")
    if volume_delta <= -0.25: negative.append(f"24h volume {volume_delta*100:.0f}%")
    if liquidity_delta <= -0.15: negative.append(f"liquidity {liquidity_delta*100:.0f}%")
    if total >= 20 and (1 - buy_ratio) >= 0.62: negative.append(f"sell pressure {(1-buy_ratio)*100:.0f}%")
    if price1h <= -8: negative.append(f"1h price {price1h:.1f}%")

    # Require multiple independent signals to reduce noise.
    about_to_move = len(positive) >= 2 and score >= 65
    about_to_drop = len(negative) >= 2
    return {
        "aboutToMove": about_to_move,
        "aboutToDrop": about_to_drop,
        "positive": positive[:5],
        "negative": negative[:5],
...
def get_goplus_access_token():
    ...
    if not GOPLUS_APP_KEY or not GOPLUS_APP_SECRET:
        print("GOPLUS_AUTH: missing App Key or App Secret")
        return ""
    ts = int(time.time())
    raw_sign = f"{GOPLUS_APP_KEY}{ts}{GOPLUS_APP_SECRET}"
    signature = hashlib.sha1(raw_sign.encode("utf-8")).hexdigest()
    ...
    try:
        req = Request(
            ...
            data=body,
            method="POST",
            headers={"Content-Type":"application/json", "Accept":"application/json", "User-Agent":"FOMO-Coin-Tracker/6.0"},
        )
        with urlopen(req, timeout=TIMEOUT) as r:
            raw = json.loads(r.read().decode("utf-8"))
        result = raw.get("result") if isinstance(raw, dict) else None
        token = result.get("access_token") if isinstance(result, dict) else None
        token = token or (result.get("token") if isinstance(result, dict) else None)
        token = token or (raw.get("access_token") if isinstance(raw, dict) else None)
        token = token or (raw.get("token") if isinstance(raw, dict) else None)
        if not token:
            print("GOPLUS_AUTH: token missing; response code=", raw.get("code") if isinstance(raw, dict) else "unknown", "message=", str(raw.get("message", ""))[:160] if isinstance(raw, dict) else "unknown")
            return ""
        GOPLUS_ACCESS_TOKEN = str(token)
        print("GOPLUS_AUTH: access token acquired")
        return GOPLUS_ACCESS_TOKEN
        ...
        try:
            ...
            detail = ""
        print(f"GOPLUS_AUTH: HTTP {exc.code} {detail}")
        return ""
    except Exception as exc:
        print("GOPLUS_AUTH: request failed:", str(exc)[:200])
        return ""
...
def goplus_security(chain, address, cached):
    ...
    # Current GoPlus docs show Bearer authentication, while GoPlus's official
    # legacy OpenAPI describes Token Security as open/permissionless. If an
    # authenticated request is rejected with the documented 4012 signature
    # error, retry the same lookup without Authorization. A failed/empty lookup
    ...
    try:
        ...
        if chain_name == "solana":
            ...
            if not chain_id:
                reason = f"Unsupported security chain: {chain}"
                print(f"GOPLUS_SECURITY: {reason}")
                return security_result_unknown(reason)
            url = f"https://api.gopluslabs.io/api/v1/token_security/{chain_id}?" + urlencode({"contract_addresses": address})

        try:
            raw = get_json(url, headers=auth_headers)
        except Exception as exc:
            print(f"GOPLUS_SECURITY: chain={chain_name} auth_request_error={type(exc).__name__}: {str(exc)[:180]}")
            raw = None

        api_code = raw.get("code") if isinstance(raw, dict) else None
        api_message = raw.get("message") if isinstance(raw, dict) else None
        print(f"GOPLUS_SECURITY: chain={chain_name} auth_code={api_code} message={str(api_message or '')[:120]}")

        # GoPlus documents 4012 as signature verification failure. The legacy
        # official OpenAPI describes Token Security as permissionless, so use a
        # no-auth fallback specifically for this failure mode.
        if api_code in (4012, "4012") or raw is None:
            try:
                raw_public = get_json(url, headers=public_headers)
                public_code = raw_public.get("code") if isinstance(raw_public, dict) else None
                public_message = raw_public.get("message") if isinstance(raw_public, dict) else None
                print(f"GOPLUS_SECURITY: chain={chain_name} public_code={public_code} message={str(public_message or '')[:120]}")
                raw = raw_public
                if public_code in (None, 1, "1"):
                    print(f"GOPLUS_SECURITY: chain={chain_name} mode=PUBLIC_FALLBACK")
            except Exception as exc:
                reason = f"GoPlus public fallback failed: {type(exc).__name__}: {str(exc)[:180]}"
                print(f"GOPLUS_SECURITY: chain={chain_name} fallback_error={reason}")
                return security_result_unknown(reason)

        api_code = raw.get("code") if isinstance(raw, dict) else None
        api_message = raw.get("message") if isinstance(raw, dict) else None
        print(f"GOPLUS_SECURITY: chain={chain_name} final_code={api_code} message={str(api_message or '')[:120]}")
        if api_code not in (None, 1, "1"):
            reason = f"GoPlus API error {api_code}: {str(api_message or '')[:120]}"
            print(f"GOPLUS_SECURITY: chain={chain_name} address_result=ERROR reason={reason}")
            return security_result_unknown(reason)
        result = raw.get("result") if isinstance(raw, dict) else None
        if not isinstance(result, dict):
            reason = "GoPlus returned no token result"
            print(f"GOPLUS_SECURITY: chain={chain_name} address_result=EMPTY_RESULT")
            return security_result_unknown(reason)
        item = result.get(address) or result.get(address.lower())
        if not isinstance(item, dict):
            # Some responses can normalize address casing or return a single object.
            dict_values = [v for v in result.values() if isinstance(v, dict)]
            item = dict_values[0] if len(dict_values) == 1 else None
        if not isinstance(item, dict):
            keys = list(result.keys())[:3] if isinstance(result, dict) else []
            print(f"GOPLUS_SECURITY: chain={chain_name} address_result=NO_MATCH result_keys={keys}")
            return security_result_unknown("GoPlus returned no matching token data")

        print(f"GOPLUS_SECURITY: chain={chain_name} address_result=DATA")
        reasons = []
        hard_block = []
        caution = []
        ...
        try:
            ...
            detail = ""
        return security_result_unknown(f"GoPlus HTTP {exc.code}: {detail}")
    except Exception as exc:
...
def publish_ntfy(topic, title, message, priority="4", tags="chart_with_upwards_trend"):
    """Publish using ntfy's JSON API so emoji/Unicode titles and messages are UTF-8 safe."""
    if not topic:
        return False
    try:
        payload = {
            ...
            "title": title,
            "message": message,
            "priority": int(priority) if str(priority).isdigit() else priority,
        ...
        req = Request(
            ...
            headers={
                ...
                "User-Agent": "FOMO-Coin-Tracker/4.0",
            },
        )
        ...
        try:
            ...
            detail = ""
        print("ntfy publish failed:", repr(exc), detail[:500])
        return False
    except Exception as exc:
        print("ntfy publish failed:", repr(exc))
        return False
...
def main():
    ...
    # Authenticate once before parallel security requests so an auth failure is
    # visible in the Actions log instead of silently producing 15 UNKNOWNs.
    if GOPLUS_APP_KEY and GOPLUS_APP_SECRET:
        get_goplus_access_token()
    else:
        print("GOPLUS_AUTH: OFF (missing GitHub secrets)")
    ...
    for item in results:
        ...
        accel = acceleration_signals(item, old)
        item["acceleration"] = accel
        next_state["pairs"][key] = {"liquidityUsd":item["liquidityUsd"],"priceUsd":item["priceUsd"],"volume24h":item["volume24h"],"buys1h":item["buys1h"],"sells1h":item["sells1h"],"opportunityScore":item["opportunityScore"],"priceChange1h":item["priceChange1h"],"riskScore":item.get("securityRisk"),"tradeSignal":item["tradeSignal"],"securityStatus":item["securityStatus"],"securityGate":item["securityGate"],"aboutToMove":bool(accel.get("aboutToMove")),"aboutToDrop":bool(accel.get("aboutToDrop"))}
        ...
        # 24/7 early-warning alerts. These are intentionally stricter than a
        # simple price pump: they require multiple independent acceleration
        # signals, and the upside alert requires the security gate to pass.
        accel = item.get("acceleration", {})
        old_move = bool(old.get("aboutToMove"))
        old_drop = bool(old.get("aboutToDrop"))
        if accel.get("aboutToMove") and not old_move and item.get("securityGate") == "PASSED":
            alert = {
                "type":"ABOUT_TO_MOVE",
                "symbol":item["symbol"],
                ...
                "signals":accel.get("positive",[]),
                "securityStatus":item.get("securityStatus"),
            }
            alerts.append(alert)
            publish_ntfy(
                NTFY_MAIN_TOPIC,
                f"🚀 ABOUT TO MOVE • {item['symbol']}",
                f"{item['name']} ({item['symbol']})\\n"
                f"This token is showing accelerating market signals.\\n"
                f"Opportunity: {item['opportunityScore']}/99\\n"
                f"Security: VERIFIED\\n"
                f"Liquidity: ${item['liquidityUsd']:,.0f}\\n"
                f"Signals: {' • '.join(accel.get('positive',[])[:4])}\\n\\n"
                f"Early-warning heuristic — not a guarantee. Review before acting.",
                "5", "rocket,chart_with_upwards_trend,white_check_mark"
            )

        if accel.get("aboutToDrop") and not old_drop:
            alert = {
                "type":"ABOUT_TO_DROP",
                "symbol":item["symbol"],
                "chainId":item["chainId"],
                "tokenAddress":item["tokenAddress"],
                "score":item["opportunityScore"],
                "signals":accel.get("negative",[]),
            }
            alerts.append(alert)
            publish_ntfy(
                NTFY_MAIN_TOPIC,
                f"⚠️ MAY BE ABOUT TO DROP • {item['symbol']}",
                f"{item['name']} ({item['symbol']})\\n"
                f"FOMO detected multiple weakening signals.\\n"
                f"Opportunity: {item['opportunityScore']}/99\\n"
                f"Liquidity: ${item['liquidityUsd']:,.0f}\\n"
                f"Signals: {' • '.join(accel.get('negative',[])[:4])}\\n\\n"
                f"Early-warning heuristic — review your position; not a guarantee.",
                "5", "warning,chart_with_downwards_trend"
            )
        if item["tradeSignal"] == "BUY SETUP" and old_signal != "BUY SETUP":
            if item["securityGate"] == "PASSED":
                ...
                message = f"{item['name']} ({item['symbol']})\nOpportunity: {item['opportunityScore']}/99\nSecurity: VERIFIED\nLiquidity: ${item['liquidityUsd']:,.0f}\n24h: {item['priceChange24h']}%\n{item['tradeReason']}\n\nRule-based signal, not a guarantee."
                publish_ntfy(topic, f"🟢 BUY SETUP • {item['symbol']}", message, "4", "chart_with_upwards_trend,green_circle")
                publish_ntfy(NTFY_MAIN_TOPIC, f"🟢 BUY SETUP • {item['symbol']}", message, "4", "chart_with_upwards_trend,green_circle")
        if item["tradeSignal"] == "EXIT / AVOID" and old_signal != "EXIT / AVOID":
            ...
            message = f"{item['name']} ({item['symbol']})\nOpportunity: {item['opportunityScore']}/99\n{item['tradeReason']}"
            publish_ntfy(topic, f"🔴 EXIT / AVOID • {item['symbol']}", message, "5", "warning,red_circle")
            publish_ntfy(NTFY_MAIN_TOPIC, f"🔴 EXIT / AVOID • {item['symbol']}", message, "5", "warning,red_circle")
    ...
    print("SECURITY_COVERAGE:", json.dumps(security_counts, separators=(",", ":")))
    print("SECURITY_CURSOR:", security_cursor, "SCANNED_THIS_RUN:", len(targets))
    ...
    print("ALERT_TYPES:", json.dumps([a.get("type") for a in alerts], separators=(",", ":")))
