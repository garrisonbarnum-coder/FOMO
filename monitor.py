...
def get_json(url, headers=None, retries=3):
    ...
    for attempt in range(retries):
        try:
            h = {"User-Agent": "Snipe/1.0", "Accept": "application/json"}
            if headers:
                h.update(headers)
...
def acceleration_signals(item, old):
    ...
    positive = []
    negative = []
    if score_delta >= 8: positive.append(f"Snipe score +{score_delta:.0f}")
    ...
    if score_delta <= -10: negative.append(f"Snipe score {score_delta:.0f}")
...
def get_goplus_access_token():
    ...
    try:
        req = Request(
            ...
            data=body,
            method="POST",
            headers={"Content-Type":"application/json", "Accept":"application/json", "User-Agent":"Snipe/1.0"},
        )
        with urlopen(req, timeout=TIMEOUT) as r:
