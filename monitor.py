...
Security provider: GoPlus Token Security API. Set GOPLUS_API_KEY as a GitHub
Actions secret. The public snapshot never contains the API key.
"""
import hashlib
import json
import math
...
def goplus_security(chain, address, cached):
    if not GOPLUS_API_KEY:
        return security_result_unknown("GOPLUS_API_KEY is not configured")
...
def main():
    ...
    # Security scan the strongest opportunities plus new/high-scoring tokens.
    targets = results[:SECURITY_TARGETS]
    for item in results:
        age = item.get("pairAgeMinutes")
        if len(targets) >= SECURITY_TARGETS + 10: break
        if isinstance(age, (int, float)) and age <= NEW_99_MAX_AGE_MINUTES and item.get("opportunityScore", 0) >= 80 and item not in targets:
            targets.append(item)
    ...
    for item in results:
        if "securityStatus" not in item:
            item.update({"securityStatus":"UNKNOWN","securityVerified":False,"securityBlocked":False,"securityRisk":None,"securityReasons":["Not selected for security scan yet"],"securitySource":"GoPlus"})
    ...
    for item in results:
        ...
        if token_id not in known_tokens:
            ...
            if not bootstrap and is_new and item.get("opportunityScore") >= 95:
    ...
    snapshot = {
        "version": 4, "generatedAt": now, "source":"DEX Screener + GoPlus",
        "monitor": {"status":"online","tokenCount":len(results),"interval":"5 minutes","securityProvider":"GoPlus","securityConfigured":bool(GOPLUS_API_KEY),"warning":"Opportunity scores are rule-based market indicators. Security verification is not a guarantee and BUY alerts are suppressed unless the security gate passes.","universe":"Established + emerging + new-token discovery feeds; not literally every token on every exchange."},
        "alerts": alerts, "alertTopics": topic_view, "tokens": results,
    }
    save_json(OUT,snapshot); save_json(STATE,next_state)
    print(f"Wrote {OUT}: {len(results)} opportunities, {len(alerts)} alerts, security={'ON' if GOPLUS_API_KEY else 'OFF'}")
